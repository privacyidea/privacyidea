...
from privacyidea.lib.authcache import (
    add_to_cache,
    delete_from_cache,
    update_cache,
    verify_in_cache,
...
class AuthCacheTestCase(MyTestCase):
    ...
    def test_01_write_update_delete_cache(self):
        ...
        r_delete = delete_from_cache(
            self.username, self.realm, self.resolver, self.password
        )
    ...
    def test_02_verify_cache(self):
        ...
        r = verify_in_cache(self.username, self.realm, self.resolver, self.password)
        self.assertFalse(r)
    ...
    def test_06_delete_other_invalid_entries(self):
        ...
        r1 = add_to_cache(
            self.username, self.realm, self.resolver, "somethingDifferent"
        )
        ...
        delete_from_cache(
            self.username, self.realm, self.resolver, self.password, max_auths=2
        )
