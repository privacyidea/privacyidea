...
from privacyidea.api.lib.utils import (
    getParam,
    check_policy_name,
    verify_auth_token,
    is_fqdn,
...
class UtilsTestCase(MyApiTestCase):
    ...
    def test_02_check_policy_name(self):
        ...
        self.assertRaises(ParameterError, check_policy_name, "Check")
        self.assertRaises(
            ParameterError, check_policy_name, "pi-update-policy-something"
        )
    ...
    def test_07_get_priority_from_param(self):
        ...
        param = {
            "priority.resolver1": 1,
            "priority.resolver2": None,
    ...
    def test_08_get_all_params(self):
        ...
        with mock.patch("logging.Logger.debug") as mock_log:
            with self.app.test_request_context(
                ...
                method="POST",
                json=params,
                headers={"Authorization": self.at, "Content-Type": "application/json"},
            ):
                res = self.app.full_dispatch_request()
                self.assertTrue(res.status_code == 200, res)
    ...
    def test_09_check_unquote(self):
        ...
        with self.app.test_request_context(
            "/auth",
            method="POST",
            data={"username": "pwpercent", "password": "pw%45#test"},
        ...
        with self.app.test_request_context(
            "/auth",
            method="POST",
            data={"username": "pwpercent", "password": quote("pw%45#test")},
            headers={"User-Agent": "some-generic-agent/1.0"},
        ):
            res = self.app.full_dispatch_request()
        ...
        with self.app.test_request_context(
            "/validate/check",
            method="POST",
            data={"user": "pwpercent", "realm": self.realm1, "pass": "pw%45#test"},
        ):
            res = self.app.full_dispatch_request()
