...
class ConditionalAccessContextTestCase(MyTestCase):
    ...
    def test_20_post_eval_uses_the_events_current_classification(self):
        ...
        # returns are recorded by the context against the row it judged. own_row_ids names the row flush() just
        # wrote, regardless of whether this request ever minted/joined an attempt_id (it did not, here) - unlike
        # the old attempt_id-keyed mechanism, which needed one before it could detect anything.
