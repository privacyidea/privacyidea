...
from privacyidea.lib.token import init_token, remove_token, check_user_pass, get_tokens
...
class LibPolicyTestCase(MyTestCase):
    ...
    def test_03_otppin_for_serial(self):
        ...
        token = init_token(
            {"serial": "T001", "type": "hotp", "genkey": 1},
            user=User("cornelius", realm="r1"),
        )
        ...
        # Correct password from userstore: "test"
        # Not identified by the user but by the token owner
        r = auth_otppin(self.fake_check_otp, token, "test", options=options, user=None)
        self.assertTrue(r)
        delete_policy("pol1")
        remove_token("T001")
    ...
    def test_07_login_mode(self):
        ...
        def check_webui_user_privacyidea(
            user_obj, password, options=None, superuser_realms=None, check_otp=False
        ):
    ...
    def test_08_config_lost_token_policy(self):
        ...
        init_token(
            {"serial": "LOST001", "type": "hotp", "genkey": 1},
            user=User("cornelius", realm="r1"),
        )
    ...
    def test_09_challenge_response_allowed(self):
        ...
        g.policy_object = PolicyClass()
        options = {"g": g}
        token = init_token({"type": "hotp", "otpkey": "1234", "pin": pin}, user=user)
    ...
    def test_10_auth_lastauth(self):
        ...
        user = User("cornelius", realm="r1")
        init_token({"type": "spass", "pin": pin, "serial": serial}, user=user)
    ...
    def test_11_otppin_with_resolvers(self):
        ...
        (added, failed) = set_realm(
            "myrealm", [{"name": "reso001"}, {"name": "reso002"}]
        )
        ...
        # match for him
        r = auth_otppin(self.fake_check_otp, None, "", options=options, user=my_user_1)
        self.assertFalse(r)

        # user in reso002 succeeds with empty PIN, since policy pol1 matches
        # for him
        r = auth_otppin(self.fake_check_otp, None, "", options=options, user=my_user_2)
        self.assertTrue(r)
    ...
    def test_13_passthru_priorities(self):
        ...
        rv = auth_user_passthru(check_user_pass, user, passw, options=options)
        self.assertTrue(rv[0])
        self.assertEqual(rv[1].get("message"), "against userstore due to 'pol3'")
    ...
    def test_14_otppin_priority(self):
        ...
        with self.assertRaises(PolicyError):
            auth_otppin(self.fake_check_otp, None, "", options=options, user=my_user)
    ...
    def test_15_reset_all_failcounters(self):
        ...
        r = reset_all_user_tokens(
            self.fake_check_token_list,
            [token1, token2],
            "pw",
            None,
            options=options,
    ...
    def test_16_passthru_assign(self):
        ...
        init_token(
            {"type": "hotp", "otpkey": "00" * 20, "serial": "TOKFAIL"},
            tokenrealms=["r1"],
        )
        init_token(
            {"type": "hotp", "otpkey": self.otpkey, "serial": "TOKMATCH"},
            tokenrealms=["r1"],
        )
