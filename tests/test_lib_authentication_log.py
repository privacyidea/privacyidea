...
class AuthenticationLogTestCase(MyTestCase):
    ...
    def test_get_authentication_logs_filter_by_peer_ip(self):
        ...
        results = get_authentication_logs(peer_ips="10.0.0.17")
        self.assertEqual(1, len(results))
        self.assertEqual("203.0.113.5", results[0].source_ip)
        self.assertSetEqual({"203.0.113.5", "203.0.113.6"},
                            {entry.source_ip for entry in get_authentication_logs(peer_ips="10.0.0.*")})
    ...
    def test_get_authentication_logs_filter_by_source_ip_source(self):
        ...
        results = get_authentication_logs(source_ip_sources="X_FORWARDED_FOR")
        self.assertEqual(1, len(results))
        self.assertEqual(2, len(get_authentication_logs(
            source_ip_sources=["X_FORWARDED_FOR", "REMOTE_ADDR"])))
    ...
    def test_get_authentication_logs_filter_by_client_label_source(self):
        ...
        results = get_authentication_logs(client_label_sources="client_id")
        self.assertEqual(1, len(results))
    ...
    def test_get_as_dict(self):
        ...
        with mock.patch("privacyidea.models.utils.datetime") as datetime_mock:
            ...
            event_id = log_authentication_event(
                ...
                endpoint="/validate/check", reasons=["TOKEN_DISABLED", "WRONG_OTP"],
        ...
        expected_keys = {"id", "resolver", "uid", "realm", "username", "user_role", "event_type", "timestamp",
                         "reasons", "source_ip", "peer_ip", "source_ip_source", "client_label",
                         "client_label_source", "ip_chain", "endpoint", "serial", "transaction_id",
...
class AuthenticationLogPaginateTestCase(MyTestCase):
    ...
    def test_visibility_scope_excluded_accounts(self):
        # The account of an excluded login is left out under whatever login its entries were recorded, and an entry
        # without a uid names no account.
        log_authentication_event(event_type=AuthEventType.LOGIN_SUCCESS, resolver="res1", uid="u-alice",
                                 realm="realm1", username="alice.before-rename")
        log_authentication_event(event_type=AuthEventType.LOGIN_SUCCESS, resolver="res2", uid="u-alice",
                                 realm="realm1", username="alice-in-another-resolver")
        log_authentication_event(event_type=AuthEventType.LOGIN_SUCCESS, resolver="res1", uid="u-bob",
                                 realm="realm1", username="bob")
        log_authentication_event(event_type=AuthEventType.USER_UNKNOWN, realm="realm1", username="nobody")
        scope = AuthenticationLogVisibilityScope(realms=[], resolvers=[], usernames=[], excluded_usernames=["alice"],
                                                 excluded_accounts=[("res1", "u-alice")])
        restricted = get_authentication_logs_paginate(visibility_scopes=[scope])
        self.assertEqual({"alice-in-another-resolver", "bob", "nobody"},
                         {entry.username for entry in restricted.auth_logs})
