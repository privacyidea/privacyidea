...
class TokenContainerManagementTestCase(MyTestCase):
    ...
    def test_28_get_all_containers_paginate(self):
        ...
        container_data = get_all_containers(ctype=["Smartphone", "Generic"], pagesize=15)
        self.assertEqual(4, container_data["count"])
        for container in container_data["containers"]:
        ...
        container_data = get_all_containers(ctype=["generic", "random_type"], pagesize=15)
        self.assertEqual(2, container_data["count"])
        for container in container_data["containers"]:
            self.assertEqual("generic", container.type)

        # Empty list returns no rows (caller explicitly requested an empty subset)
        container_data = get_all_containers(ctype=[], pagesize=15)
