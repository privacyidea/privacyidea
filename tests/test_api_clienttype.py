...
class APIClienttypeTestCase(MyApiTestCase):
    def test_00_get_client(self):
        ...
        with self.app.test_request_context(
            "/client/", method="GET", headers={"Authorization": self.at}
        ):
            ...
            self.assertTrue("RADIUS" in json_response.get("result").get("value"))

        # test the decorator
        with self.app.test_request_context(
            "/validate/check",
            method="POST",
            data={"user": "hans", "pass": "dampf"},
        ...
        with self.app.test_request_context(
            ...
            self.assertTrue("PAM" in json_response.get("result").get("value"))
            self.assertTrue("RADIUS" in json_response.get("result").get("value"))
            self.assertTrue("unknown" in json_response.get("result").get("value"))
