...
class TwoStepInitTestCase(MyApiTestCase):
    ...
    def test_01_init_token(self):
        ...
        with self.app.test_request_context(
            "/token/init",
            method="POST",
            data={"type": "hotp", "genkey": "1", "2stepinit": "1"},
        ...
        with self.app.test_request_context(
            ...
            result = res.json.get("result")
            self.assertIn(
                "Malformed base32check data: Incorrect checksum",
                result.get("error").get("message"),
            )
        ...
        with self.app.test_request_context(
            "/validate/check",
            method="POST",
            data={"serial": serial, "pass": wrong_otp_value},
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
            result = res.json
    ...
    def test_05_init_totp_token(self):
        ...
        with self.app.test_request_context(
            "/token/init",
            method="POST",
            data={"type": "totp", "genkey": "1", "2stepinit": "1"},
            headers={"Authorization": self.at},
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
