...
from privacyidea.lib.tokens.pushtoken import (
    PushTokenClass,
    PUSH_ACTION,
...
TTL = 10
FB_CONFIG_VALS = {FIREBASE_CONFIG.JSON_CONFIG: FIREBASE_FILE}
...
class PushTokenTestCase(MyTestCase):
    ...
    def test_01_create_token(self):
        ...
        self.assertRaises(
            ParameterError,
            token.update,
            {"otpkey": "1234", "pubkey": "1234", "serial": self.serial1},
        )
        ...
        self.assertEqual(
            token.get_tokeninfo("public_key_smartphone"),
        ...
        with mock.patch(
            ...
            with mock.patch("logging.Logger.warning") as mock_log:
                with self.app.test_request_context(
                    "/validate/check",
                    method="POST",
                    data={"user": "cornelius", "realm": self.realm1, "pass": "pushpin"},
                ):
    ...
    def test_03b_api_authenticate_client(self):
        ...
        # We mock the ServiceAccountCredentials, since we can not directly contact the Google API
        with mock.patch(
            "privacyidea.lib.smsprovider.FirebaseProvider.service_account"
        ...
        with mock.patch(
            "privacyidea.lib.smsprovider.FirebaseProvider.time"
        ) as mock_time:
            ...
            self.assertIn(FIREBASE_FILE, get_app_local_store()["firebase_token"])
            self.assertEqual(
                get_app_local_store()["firebase_token"][FIREBASE_FILE].access_token,
                "my_new_bearer_token",
            )
    ...
    def test_04_api_authenticate_smartphone(self):
        ...
        with self.app.test_request_context(
            "/ttype/push",
            method="POST",
            data={"serial": tokenobj.token.serial, "nonce": challenge},
        ):
            res = self.app.full_dispatch_request()
        ...
        with self.app.test_request_context(
            "/ttype/push",
            method="POST",
            data={"serial": tokenobj.token.serial, "nonce": "", "signature": signature},
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
            self.assertTrue(res.json["result"]["status"])
    ...
    def test_04_decline_auth_request(self):
        ...
        with self.app.test_request_context(
            ...
            self.assertFalse(res.json["result"]["value"])
            self.assertEqual(
                res.json["detail"]["challenge_status"],
                "declined",
                res.json["detail"]["challenge_status"],
            )
    ...
    def test_15_poll_endpoint(self):
        ...
        with mock.patch("privacyidea.models.datetime") as mock_dt1, mock.patch(
            "privacyidea.lib.tokens.pushtoken.datetime"
        ) as mock_dt2:
