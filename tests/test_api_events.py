...
class APIEventsTestCase(MyApiTestCase):
    def test_00_api_errors(self):
        ...
        with self.app.test_request_context("/event/", method="GET"):
    ...
    def test_02_test_options(self):
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(event_list[0].get("options").get("emailconfig"), "themis")
    ...
    def test_05_get_handler_actions(self):
        with self.app.test_request_context(
            "/event/actions/UserNotification",
            method="GET",
            headers={"Authorization": self.at},
        ):
            res = self.app.full_dispatch_request()
    ...
    def test_05_get_handler_conditions(self):
        with self.app.test_request_context(
            "/event/conditions/UserNotification",
            method="GET",
            headers={"Authorization": self.at},
        ):
            res = self.app.full_dispatch_request()
    ...
    def test_08_create_token_for_user(self):
        ...
        with self.app.test_request_context(
            "/token/",
            data={"user": "usernotoken", "realm": self.realm2},
            method="GET",
            headers={"Authorization": self.at},
        ):
            res = self.app.full_dispatch_request()
        ...
        with self.app.test_request_context(
            "/validate/check",
            data={"user": "usernotoken", "realm": self.realm2, "pass": "1234"},
            method="POST",
            headers={"Authorization": self.at},
        ):
            res = self.app.full_dispatch_request()
...
class CustomUserAttributeHandlerTestCase(MyApiTestCase):
    ...
    def test_01_user_attribute_with_handler_tokenowner(self):
        ...
        with self.app.test_request_context(
            "/validate/check",
            data={"serial": tok.token.serial, "pass": "test"},
            method="POST",
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
...
class EventWrapperTestCase(MyApiTestCase):
    ...
    def test_01_sendmail_post(self):
        ...
        smtpmock.setdata(
            response={"pretzel@example.com": (450, "Mailbox not available")},
            support_tls=False,
        )
        ...
        with mock.patch("logging.Logger.warning") as mock_log:
            with self.app.test_request_context(
                "/token/init",
                data={"genkey": 1, "serial": self.serial},
                headers={"Authorization": self.at},
                method="POST",
            ):
                res = self.app.full_dispatch_request()
    ...
    def test_02_sendmail_pre(self):
        ...
        smtpmock.setdata(
            response={"donut@example.com": (450, "Mailbox not available")},
            support_tls=False,
        )
