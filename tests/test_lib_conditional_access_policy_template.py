...
class ConditionalAccessPolicyTemplateTestCase(MyTestCase):
    ...
    def tearDown(self):
        for model in (ConditionalAccessStageAction, ConditionalAccessPolicyStage, ConditionalAccessPolicyCounterType, ConditionalAccessPolicy):
...
class ConditionalAccessTemplateBehaviourTestCase(ConditionalAccessTestCase):
        ...
        *enforce* a template shipped as ``dry_run`` is turned on (simulating an
        ...
        if configure_email:
            for stage in policy["stages"]:
                for action in stage["actions"]:
                    if action["action_type"] in (ConditionalAccessAction.EMAIL_ADMIN, ConditionalAccessAction.EMAIL_USER):
    ...
    def test_ip_failed_rate_limiting_ships_dry_run(self):
        ...
        self.assertTrue(get_conditional_access_policy(policy_id)["dry_run"], "ip_failed_rate_limiting must ship as dry-run")
