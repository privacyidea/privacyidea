...
from privacyidea.lib.policy import set_policy, SCOPE, ACTION, REMOTE_USER, delete_policy
...
class AuthApiTestCase(MyApiTestCase):
    def test_01_auth_with_split(self):
        ...
        with self.app.test_request_context(
            "/auth",
            method="POST",
            data={"username": "cornelius@realm1", "password": "test"},
        ):
            res = self.app.full_dispatch_request()
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(4031, result["error"]["code"], result)
            self.assertEqual(
                "Authentication failure. Unknown realm: unknown.",
                result["error"]["message"],
                result,
            )
        ...
        with self.app.test_request_context(
            "/auth",
            method="POST",
            data={"username": "cornelius", "realm": "realm1", "password": "test"},
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(200, res.status_code, res)
            result = res.json.get("result")
        ...
        with self.app.test_request_context(
            "/auth",
            method="POST",
            data={"username": "cornelius", "realm": "unknown", "password": "test"},
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(401, res.status_code, res)
            result = res.json.get("result")
        ...
        with self.app.test_request_context(
            "/auth",
            method="POST",
            data={"username": "cornelius", "realm": "", "password": "test"},
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(200, res.status_code, res)
            result = res.json.get("result")
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(4031, result["error"]["code"], result)
            self.assertEqual(
                "Authentication failure. Unknown realm: anotherunknown.",
                result["error"]["message"],
                result,
            )
    ...
    def test_03_admin_auth(self):
        ...
        with self.app.test_request_context(
            "/auth",
            method="POST",
            data={"username": "super@intern", "password": "testing"},
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(200, res.status_code, res)
            result = res.json.get("result")
        ...
        with mock.patch("logging.Logger.warning") as mock_log:
            with self.app.test_request_context(
                ...
                mock_log.assert_called_once_with(
                    "Problem resolving user testadmin in "
    ...
    def test_06_auth_user_not_in_defrealm(self):
        ...
        with self.app.test_request_context(
            "/auth",
            method="POST",
            data={"username": "selfservice@realm1", "password": "test", "realm": ""},
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(res.status_code, 200, res)
            result = res.json.get("result")
    ...
    def test_07_user_not_in_userstore(self):
        ...
        with self.app.test_request_context(
            "/auth",
            method="POST",
            data={"username": "user-really-does-not-exist", "password": "test"},
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(401, res.status_code, res)
            result = res.json.get("result")
    ...
    def test_01_admin_and_user_same_name(self):
        ...
        with mock.patch("logging.Logger.warning") as mock_log:
            with self.app.test_request_context(
                "/auth",
                method="POST",
                data={"username": "testadmin", "password": "unknown"},
            ):
                res = self.app.full_dispatch_request()
                self.assertEqual(401, res.status_code, res)
                result = res.json.get("result")
...
class EventHandlerTest(MyApiTestCase):
    ...
    def test_02_post_eventhandler(self, capture):
        ...
        capture.check_present(("pi-eventlogger", "INFO", "User: someuser Event: /auth"))
