...
class TokenModelTestCase(MyTestCase):
    ...
    def test_01_write_stats(self):
        key1 = "some_key"
        write_stats(current_app.config, key1, 12)
        write_stats(current_app.config, key1, 13)
        ...
        write_stats(current_app.config, key1, 14, reset_values=True)
        self.assertEqual(MonitoringStats.query.filter_by(stats_key=key1).count(), 1)

    def test_02_delete_stats(self):
        key1 = "otherkey"
        now = datetime.datetime.now(tzlocal())
        write_stats(current_app.config, key1, 12, timestamp=now- timedelta(days=1))
        write_stats(current_app.config, key1, 13, timestamp=now)
        write_stats(current_app.config, key1, 14, timestamp=now + timedelta(days=1))
        self.assertEqual(MonitoringStats.query.filter_by(stats_key=key1).count(), 3)

        # delete the last two entries
        r = delete_stats(current_app.config, key1, start_timestamp=now - timedelta(minutes=60))
        ...
        r = delete_stats(current_app.config, key1, end_timestamp=now + timedelta(minutes=60))
        self.assertEqual(MonitoringStats.query.filter_by(stats_key=key1).count(), 1)
        self.assertEqual(r, 2)

    def test_03_get_keys(self):
        # delete old entries
        keys = get_stats_keys(current_app.config)
        for k in keys:
            delete_stats(current_app.config, k)

        # write new stats entries
        write_stats(current_app.config, "key1", 13)
        write_stats(current_app.config, "key1", 13)
        write_stats(current_app.config, "key1", 13)
        write_stats(current_app.config, "key2", 12)
        write_stats(current_app.config, "key3", 12)
    ...
    def test_04_get_values(self):
        ...
        write_stats(current_app.config, "key1", 1, timestamp=ts - timedelta(minutes=10))
        write_stats(current_app.config, "key1", 2, timestamp=ts - timedelta(minutes=9))
        write_stats(current_app.config, "key1", 3, timestamp=ts - timedelta(minutes=8))
        write_stats(current_app.config, "key1", 4, timestamp=ts - timedelta(minutes=7))
        write_stats(current_app.config, "key1", 5, timestamp=ts - timedelta(minutes=6))
        write_stats(current_app.config, "key1", 6, timestamp=ts - timedelta(minutes=5))
        write_stats(current_app.config, "key1", 7, timestamp=ts - timedelta(minutes=4))
        write_stats(current_app.config, "key1", 8, timestamp=ts - timedelta(minutes=3))
        write_stats(current_app.config, "key1", 9, timestamp=ts - timedelta(minutes=2))
        write_stats(current_app.config, "key1", 10, timestamp=ts - timedelta(minutes=1))

        r = get_values(current_app.config, "key1")
        ...
        r = get_last_value(current_app.config, "key1")
