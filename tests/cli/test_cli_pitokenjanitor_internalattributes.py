...
from privacyidea.cli.pitokenjanitor.utils import findinternalattributes
...
class TestInternalAttributes:
    ...
    def test_runs_without_an_application_context(self, app):
        ...
        with app.app_context():
            db.session.add(InternalUserAttribute(user_id="some-uid", resolver="ghost-resolver", realm_id=None,
                                                 Key="some_key", Value="v"))
            db.session.commit()

        result = CliRunner().invoke(cli, ["internal-attributes", "list"],
    ...
    def test_delete_help_does_not_scan(self, app, monkeypatch):
        ...
        monkeypatch.setattr(findinternalattributes, "find_orphaned_internal_attributes", fail)
        result = CliRunner().invoke(cli, ["internal-attributes", "delete", "--help"],
                                    obj=ScriptInfo(create_app=lambda: app))
        assert result.exit_code == 0, result.output
