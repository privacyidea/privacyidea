...
def attributes(app, realm):
    """A live user with a custom attribute + an orphan row whose uid the resolver has never seen."""
    live = User(login="cornelius", realm="realm1")
    live.set_attribute("test_key", "live-value")

    ghost = CustomUserAttribute(user_id="ghost-9999",
...
class TestCustomAttributes:
    def test_list_reports_orphans(self, app, attributes):
        ...
        result = runner.invoke(cli, ["custom-attributes", "list"])
    ...
    def test_default_subcommand_is_list(self, app, attributes):
        ...
        runner = app.test_cli_runner()
        result = runner.invoke(cli, ["custom-attributes"])
        assert result.exit_code == 0, result.output
        assert "ghost-9999" in result.output

    def test_delete_with_yes_removes_orphans(self, app, attributes):
        runner = app.test_cli_runner()
        result = runner.invoke(cli, ["custom-attributes", "delete", "--yes"])
        assert result.exit_code == 0, result.output
    ...
    def test_orphan_when_resolver_was_deleted(self, app, realm):
        ...
        row = CustomUserAttribute(user_id="some-uid",
