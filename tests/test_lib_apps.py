...
from privacyidea.lib.apps import (
...
class AppsTestCase(MyTestCase):
    def test_01_apps_urls(self):
        ...
        self.assertTrue("otpauth://hotp/mylabel?secret=CI2FM6A&counter=1" in r, r)

        r = create_google_authenticator_url(tokentype="TOTP", key="123456", period=60)
        self.assertTrue(
            "otpauth://totp/mylabel?secret=CI2FM&period"
            "=60&digits=6&creator=privacyidea&issuer=privacyIDEA" in r,
            r,
        )
        ...
        self.assertEqual(
            r,
            "oathtoken:///addToken?name=mylabel&"
