...
from privacyidea.lib.utils.compare import (
    compare_values,
    CompareError,
...
class UtilsCompareTestCase(MyTestCase):
    ...
    def test_04_compare_matches(self):
        ...
        self.assertTrue(
            compare_values(
                "uid=hello,cn=users,dc=test,dc=intranet",
                "matches",
        ...
        self.assertFalse(
            compare_values(
                "uid=hello,cn=users,dc=test,dc=intranet,dc=world",
                "matches",
                "uid=[^,]+,cn=users,dc=test,dc=intranet",
            )
        )
        ...
        self.assertFalse(
            compare_values(
                "uid=hello,cn=admins,dc=test,dc=intranet",
                "!matches",
                "uid=[^,]+,cn=admins,dc=test,dc=intranet",
            )
        )
