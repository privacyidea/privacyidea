...
class SMSTestCase(MyTestCase):
    ...
    def test_02_create_modify_delete_smsgateway_configuration(self):
        ...
        id = set_smsgateway(
            ...
            provider_module,
            description="test",
            options={"HTTP_METHOD": "POST", "URL": "example.com"},
            headers={"Authorization": "QWERTZ", "BANANA": "will be eaten"},
        )
...
class SmtpSMSTestCase(MyTestCase):
    ...
    identifier_config = {"MAILTO": "recp@example.com", "IDENTIFIER": "myServer"}
...
class HttpSMSTestCase(MyTestCase):
    ...
    config_get = {
        "URL": get_url,
        "PARAMETER": {"user": "username", "password": "askme", "api_id": "12980436"},
        "SMS_TEXT_KEY": "text",
    ...
    def test_04_send_sms_get_fail(self):
        ...
        # Here we need to send the SMS
        self.assertRaises(SMSError, self.get_provider.submit_message, "123456", "Hello")
    ...
    def test_10_new_smsgateway(self):
        ...
        responses.add(responses.POST, "http://example.com", body=self.success_body)
        # Here we need to send the SMS
        r = sms.submit_message("123456", "Hello")
        self.assertTrue(r)
    ...
    def test_12_send_sms_post_success_as_json(self):
        ...
        responses.add(
            ...
            match=[json_params_matcher({"text": "Hello: 7", "phone": "123456"})],
        )
        # Here we need to send the SMS
...
class SmppSMSTestCase(MyTestCase):
    ...
    provider_module = "privacyidea.lib.smsprovider.SmppSMSProvider" ".SmppSMSProvider"
    ...
    def test_01_success(self):
        # Here we need to send the SMS
        smppmock.setdata(
            connection_success=True, systemid="privacyIDEA", password="secret"
        )
        r = self.provider.submit_message("123456", "Hello")
        self.assertTrue(r)
    ...
    def test_02_fail_connection(self):
        smppmock.setdata(
            connection_success=False, systemid="privacyIDEA", password="secret"
        )
        self.assertRaises(SMSError, self.provider.submit_message, "123456", "hello")
    ...
    def test_03_fail_wrong_credentials(self):
        smppmock.setdata(
            connection_success=True, systemid="privacyIDEA", password="wrong"
        )
        self.assertRaises(SMSError, self.provider.submit_message, "123456", "hello")
