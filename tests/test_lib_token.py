...
from privacyidea.lib.policy import set_policy, SCOPE, ACTION, PolicyClass, delete_policy
...
class TokenTestCase(MyTestCase):
    ...
    def test_01_create_token(self):
        for serial in self.serials:
            ...
            self.assertTrue(token_object.mode[0] == "authenticate", token_object.mode)
            self.assertTrue(token_object.mode[1] == "challenge", token_object.mode)
    ...
    def test_02_get_tokens(self):
        ...
        for token_object in tokenobject_list:
            self.assertTrue(token_object.type in get_token_types(), token_object.type)
        ...
        self.assertTrue(tokenobject_list[0].type == "hotp", tokenobject_list[0].type)
        ...
        self.assertEqual(
            tokenobject_list[0].get_tokeninfo("yubikey.prefix"), "vv123456"
        )
        remove_token("yk1")

        # Tokeninfo with more than one entry is not supported
        self.assertRaises(
            privacyIDEAError, get_tokens, tokeninfo={"key1": "value1", "key2": "value2"}
        )
    ...
    def test_08_token_owner(self):
        ...
        self.assertTrue(is_token_owner("hotptoken", user), get_token_owner("hotptoken"))
        self.assertFalse(
            is_token_owner("hotptoken", User()), get_token_owner("hotptoken")
        )
        self.assertFalse(
    ...
    def test_11_get_otp(self):
        ...
        otp = get_otp(
            self.serials[0], current_time=datetime.datetime(2014, 12, 4, 12, 0)
        )
    ...
    def test_12_get_token_by_otp(self):
        ...
        db_token.update_otpkey(self.otpkey)
        db_token.save()
        self.assertRaises(TokenAdminError, get_serial_by_otp, get_tokens(), "287922")
        db_token.delete()
    ...
    def test_15_init_token(self):
        ...
        self.assertTrue(tokenobject.token.tokentype == "hotp", tokenobject.token)
        ...
        self.assertRaises(
            TokenAdminError, init_token, {"otpkey": "1234", "type": "never_know"}
        )
        ...
        self.assertRaises(
            TokenAdminError,
            init_token,
            {"otpkey": "1234", "serial": "NEW001", "type": "totp"},
        )
        ...
        self.assertTrue(
            self.realm1 in tokenobject.token.get_realms(),
            tokenobject.token.get_realms(),
        )

        tokenobject = init_token(
            {"serial": "NEW003", "type": "hotp", "otpkey": "1234567890123456"},
    ...
    def test_16_remove_token(self):
        ...
        tokenobject = init_token(
            {"type": "hotp", "otpkey": "1234567890123456", "realm": self.realm1}
        )
        ...
        chall_count = Challenge.query.filter(
            Challenge.serial == tokenobject.get_serial()
        ).count()
    ...
    def test_17_set_defaults(self):
        serial = "SETTOKEN"
        tokenobject = init_token(
            {"serial": serial, "otpkey": "1234567890123456", "otplen": 8}
        )
    ...
    def test_18_assign_token(self):
        ...
        self.assertRaises(
            TokenAdminError, assign_token, serial, User("shadow", realm=self.realm1)
        )
    ...
    def test_21_enable_disable(self):
        ...
        self.assertTrue(tokenobject.token.active == False, tokenobject.token.active)
    ...
    def test_26_set_sync_window(self):
        ...
        self.assertTrue(r == 1, r)
        self.assertTrue(
            tokenobject.token.sync_window == 23, tokenobject.token.sync_window
        )
        remove_token(serial)
    ...
    def test_27_set_count_window(self):
        ...
        self.assertTrue(r == 1, r)
        self.assertTrue(
            tokenobject.token.count_window == 45, tokenobject.token.count_window
        )
        remove_token(serial)
    ...
    def test_28_set_description(self):
        ...
        self.assertTrue(r == 1, r)
        self.assertTrue(
            tokenobject.token.description == "new description",
    ...
    def test_34_check_token_list(self):
        ...
        self.assertEqual(failcount, old_failcount + 1, (old_failcount, failcount))
        ...
        self.assertTrue(
            old_counter < hotp_tokenobject_new.token.count,
            (old_counter, hotp_tokenobject.token.count),
        )
        ...
        res, reply = check_token_list(
            tokenobject_list, "481090", options={"transaction_id": transaction_id}
        )
        self.assertTrue(res)
        ...
        self.assertEqual(
            reply.get("message"), "No active challenge response token found"
        )
    ...
    def test_37_challenge(self):
        ...
        r, reply = check_serial_pass(
            "hotptoken", "436521", {"transaction_id": transaction_id}
        )
        self.assertTrue(r)
        self.assertTrue(reply.get("message") == "Found matching challenge", reply)
    ...
    def test_41_get_tokens_paginate(self):
        ...
        self.assertTrue(len(tokens.get("tokens")) == 5, len(tokens.get("tokens")))
        ...
        self.assertTrue(len(tokens.get("tokens")) == 2, len(tokens.get("tokens")))
        self.assertTrue(tokens.get("count") == 2, tokens.get("count"))
        self.assertTrue(tokens.get("next") is None, tokens.get("next"))
        self.assertTrue(tokens.get("prev") is None, tokens.get("prev"))

        tokens = get_tokens_paginate(sortby=Token.serial, page=1, sortdir="desc")
    ...
    def test_42_sort_tokens(self):
        ...
        tokendata = get_tokens_paginate(
            sortby=Token.serial, page=1, psize=100, sortdir="asc"
        )
        self.assertTrue(len(tokendata.get("tokens")) >= 9, len(tokendata.get("tokens")))
        ...
        # Reverse sorting
        tokendata = get_tokens_paginate(
            sortby=Token.serial, page=1, psize=100, sortdir="desc"
        )
        tokens = tokendata.get("tokens")
        ...
        tokendata = get_tokens_paginate(
            sortby="serial", page=1, psize=100, sortdir="asc"
        )
        tokens = tokendata.get("tokens")
        ...
        tokendata = get_tokens_paginate(
            sortby="serial", page=1, psize=100, sortdir="desc"
        )
        tokens = tokendata.get("tokens")
    ...
    def test_43_encryptpin(self):
        serial = "ENC01"
        # encrypt pin on init
        init_token({"serial": serial, "genkey": 1, "pin": "Hallo", "encryptpin": True})
    ...
    def test_44_validity_period(self):
        serial = "VAL01"
        init_token({"serial": serial, "genkey": 1, "pin": "Hallo"})
        tokenobj = get_tokens(serial=serial)[0]
    ...
    def test_45_check_realm_pass(self):
        ...
        serial = "inactive"
        init_token(
            {"serial": serial, "otpkey": self.otpkey, "pin": serial},
        ...
        tokens = get_tokens(
            realm=self.realm1, tokentype="hotp", assigned=False, serial="not_assigned"
        )
        self.assertEqual(len(tokens), 1)

        tokens = get_tokens(
            realm=self.realm1, tokentype="hotp", active=False, serial="inactive"
        )
        self.assertEqual(len(tokens), 1)
        ...
        self.assertEqual(
            r[1].get("message"),
            "There is no active and assigned " "token in this realm",
        )

        # check for optional parameter exclude/include type
        r = check_realm_pass(
            self.realm1, "assigned" + self.valid_otp_values[2], exclude_types="hotp"
        )
        self.assertFalse(r[0])
        self.assertEqual(
            r[1].get("message"),
            "There is no active and assigned "
            "token in this realm, included types: None, "
            "excluded types: hotp",
        )

        r = check_realm_pass(
            self.realm1, "assigned" + self.valid_otp_values[2], include_types="totp"
        )
        self.assertFalse(r[0])
        self.assertEqual(
            r[1].get("message"),
        ...
        r = check_realm_pass(
            self.realm1, "assigned" + self.valid_otp_values[2], exclude_types="totp"
        )
        self.assertTrue(r[0])
        self.assertEqual(r[1].get("message"), "matching 1 tokens")
        ...
        r = check_realm_pass(
            self.realm1,
            "assigned" + self.valid_otp_values[3],
            include_types="hotp",
            exclude_types="hotp",
        )
        self.assertTrue(r[0])
    ...
    def test_47_use_yubikey_and_hotp(self):
        ...
        init_token(
            {"type": "yubikey", "otpkey": self.yubikey_token_key, "pin": "pin47"}, user
        )
    ...
    def test_48_challenge_request_two_tokens(self):
        ...
        r, r_dict = check_token_list(
            [token_a, token_b],
            self.valid_otp_values[1],
            user,
    ...
    def test_50_otpkeyformat(self):
        ...
        self.assertTrue(tokenobject.token.tokentype == "hotp", tokenobject.token)
        self.assertEqual(
            tokenobject.token.get_otpkey().getKey(), binascii.hexlify(otpkey)
        )
        remove_token("NEW001")
        # unknown encoding
    ...
    def test_51_tokenkind(self):
        ...
        tok = init_token(
            {"type": "totp", "otpkey": self.otpkey}, tokenkind=TOKENKIND.HARDWARE
        )
        kind = tok.get_tokeninfo("tokenkind")
        ...
        tok = init_token(
            {"type": "hotp", "otpkey": self.otpkey, "serial": "UBOM111111"}
        )
        kind = tok.get_tokeninfo("tokenkind")
        self.assertEqual(kind, TOKENKIND.HARDWARE)
        tok.delete_token()
        ...
        tok = init_token({"type": "yubico", "yubico.tokenid": "123456789012"})
        kind = tok.get_tokeninfo("tokenkind")
        self.assertEqual(kind, TOKENKIND.HARDWARE)
        tok.delete_token()
        ...
        tok = init_token(
            {"type": "radius", "radius.identifier": "1", "radius.user": "hans"}
        )
        kind = tok.get_tokeninfo("tokenkind")
        ...
        tok = init_token(
            {"type": "remote", "remote.server": "1", "radius.user": "hans"}
        )
        kind = tok.get_tokeninfo("tokenkind")
        self.assertEqual(kind, TOKENKIND.VIRTUAL)
        tok.delete_token()

        tok = init_token({"type": "4eyes", "4eyes": "realm1", "separator": ","})
        kind = tok.get_tokeninfo("tokenkind")
        self.assertEqual(kind, TOKENKIND.VIRTUAL)
        tok.delete_token()
    ...
    def test_54_helper_functions(self):
        ...
        self.assertEqual(
            get_one_token(serial="hotptoken", user=None).token.serial, "hotptoken"
        )
        ...
        self.assertEqual(
            len(get_tokens_from_serial_or_user(serial=None, user=shadow)), 0
        )
        ...
        self.assertEqual(
            len(get_tokens_from_serial_or_user(serial="hotptoken", user=None)), 1
        )
        with self.assertRaises(ResourceNotFoundError):
    ...
    def test_55_get_tokens_paginated_generator(self):
        ...
        self.assertEqual(
            list(get_tokens_paginated_generator(serial_wildcard="serial*")), [[]]
        )
    ...
    def test_57a_check_invalid_serial(self):
        ...
        self.assertRaises(
            Exception, init_token, {"serial": "invalid/chars", "genkey": 1}
        )
    ...
    def test_59_weigh_token_types(self):
        ...
        self.assertTrue(
            weigh_token_type(dummy_token("push"))
            > weigh_token_type(dummy_token("hotp"))
        )
        self.assertTrue(
            weigh_token_type(dummy_token("push"))
            > weigh_token_type(dummy_token("HOTP"))
...
class TokenOutOfBandTestCase(MyTestCase):
    ...
    def test_01_failcounter_no_increase(self):
        ...
        pin1 = "pin1"
        token1 = init_token(
            {"serial": pin1, "pin": pin1, "type": "tiqr", "genkey": 1}, user=user
        )
        ...
        for i in range(1, 10):
            r, r_dict = check_token_list(
                [token1], "", user=user, options={"transaction_id": transaction_id}
            )
            self.assertFalse(r, r_dict)
        ...
        Challenge.query.filter(Challenge.transaction_id == transaction_id).update(
            {"otp_valid": True}
        )
        r, r_dict = check_token_list(
            [token1], "", user=user, options={"transaction_id": transaction_id}
...
class TokenFailCounterTestCase(MyTestCase):
    ...
    def test_01_failcounter_max_hotp(self):
        ...
        user = User(login="cornelius", realm=self.realm1)
        token = init_token(
            {"serial": "test47", "pin": "test47", "type": "hotp", "otpkey": OTPKEY},
            user=user,
        )
        ...
        self.assertFalse(res)
        self.assertEqual(
            reply.get("message"), "matching 1 tokens, " "Failcounter exceeded"
        )
    ...
    def test_02_failcounter_max_totp(self):
        ...
        pin = "testTOTP"
        token = init_token(
            {"serial": pin, "pin": pin, "type": "totp", "otpkey": OTPKEY}, user=user
        )
        """
        47251644    942826
        47251645    063321
        ...
        47251650    589836
        """
        res, reply = check_user_pass(
            user, pin + "942826", options={"initTime": 47251644 * 30}
        )
        self.assertTrue(res)
        # Set the failcounter to maximum failcount
        token.set_failcount(10)
        # Authentication must fail, since the failcounter is reached
        res, reply = check_user_pass(
            user, pin + "032819", options={"initTime": 47251648 * 30}
        )
        self.assertFalse(res)
        self.assertEqual(
            reply.get("message"), "matching 1 tokens, " "Failcounter exceeded"
    ...
    def test_03_inc_failcounter_of_all_tokens(self):
        ...
        pin2 = "pin2"
        token1 = init_token(
            {"serial": pin1, "pin": pin1, "type": "hotp", "genkey": 1}, user=user
        )
        token2 = init_token(
            {"serial": pin2, "pin": pin2, "type": "hotp", "genkey": 1}, user=user
        )
    ...
    def test_04_reset_all_failcounters(self):
        from privacyidea.lib.policy import set_policy, PolicyClass, SCOPE, ACTION
        from flask import g
        ...
        pin1 = "pin1"
        pin2 = "pin2"
        token1 = init_token({"serial": pin1, "pin": pin1, "type": "spass"}, user=user)
        token2 = init_token({"serial": pin2, "pin": pin2, "type": "spass"}, user=user)
        ...
        check_token_list(
            [token1, token2], pin1, options=options, allow_reset_all_tokens=True
        )
    ...
    def test_05_reset_failcounter(self):
        tok = init_token({"type": "hotp", "serial": "test05", "otpkey": self.otpkey})
        # Set failcounter clear timeout to 1 minute
        set_privacyidea_config(FAILCOUNTER_CLEAR_TIMEOUT, 1)
        tok.token.count = 10
        tok.set_pin("hotppin")
        tok.set_failcount(10)
        exceeded_timestamp = datetime.datetime.now(tzlocal()) - datetime.timedelta(
            minutes=1
        )
        tok.add_tokeninfo(
            FAILCOUNTER_EXCEEDED, exceeded_timestamp.strftime(DATE_FORMAT)
        )
    ...
    def test_06_reset_failcounter_out_of_sync(self):
        ...
        tok = init_token({"type": "hotp", "serial": "test06", "otpkey": self.otpkey})
    ...
    def test_07_reset_failcounter_on_pin_only(self):
        tok = init_token({"type": "hotp", "serial": "test07", "otpkey": self.otpkey})
        # Set failcounter clear timeout to 1 minute
        set_privacyidea_config(FAILCOUNTER_CLEAR_TIMEOUT, 1)
        tok.token.count = 10
        tok.set_pin("hotppin")
...
class PINChangeTestCase(MyTestCase):
    ...
    def test_00_create_realms(self):
        ...
        set_policy(
            "viaValidate", scope=SCOPE.AUTH, action=ACTION.CHANGE_PIN_VIA_VALIDATE
        )
    ...
    def test_01_successfully_change_pin(self):
        ...
        r, reply_dict = check_token_list(
            [tok, tok2], "test", user=user_obj, options={"g": g}
        )
        self.assertFalse(r)
        ...
        r, reply_dict = check_token_list(
            ...
            self.valid_otp_values[1],
            user=user_obj,
            options={"transaction_id": transaction_id, "g": g},
        )
        self.assertFalse(r)
        self.assertEqual("Please enter a new PIN", reply_dict.get("message"))
        transaction_id = reply_dict.get("transaction_id")
        self.assertEqual(
            "interactive", reply_dict.get("multi_challenge")[0].get("client_mode")
        )
    ...
    def test_03_failed_change_pin(self):
        ...
        with warnings.catch_warnings():
            ...
            self.assertRaisesRegex(
                PolicyError,
                "The minimum OTP PIN length is 5",
                check_token_list,
                [tok, tok2],
                newpin,
                user=user_obj,
...
class ExportAndReencryptTestCase(MyTestCase):
    def test_00_create_tokens(self):
        ...
        for t in tokens:
            init_token({"type": t[1], "serial": t[0], "otpkey": t[2], "timeStep": 60})
    ...
    def test_01_token_dump_load(self):
        ...
        pin = "1234"
        tokenobject = init_token(
            {"serial": "s2", "pin": pin, "otpkey": key}, user=User("hans", self.realm1)
        )
