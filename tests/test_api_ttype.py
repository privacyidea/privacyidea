...
class TtypeAPITestCase(MyApiTestCase):
    ...
    def test_01_tiqr(self):
        init_token({"serial": "TIQR1", "type": "tiqr"}, User("cornelius", self.realm1))
        with self.app.test_request_context(
            "/ttype/tiqr",
            method="POST",
            data={"action": "metadata", "serial": "TIQR1", "session": "12345"},
        ):
...
class TtypePushAPITestCase(MyApiTestCase):
    ...
    def test_01_api_enroll_push(self):
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(
                tokeninfo.get("public_key_smartphone"),
    ...
    def test_02_api_push_poll(self):
        ...
        with self.app.test_request_context(
            ...
            with self.app.test_request_context(
                "/ttype/push",
                method="POST",
                data={"serial": serial, "nonce": challenge, "signature": signature},
            ):
                res = self.app.full_dispatch_request()
                self.assertTrue(res.status_code == 200, res)
                self.assertTrue(res.json["result"]["status"])
