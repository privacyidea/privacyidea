...
class LdapMachineTestCase(MyTestCase):
    ...
    mrAD = LdapMachineResolver(
        "myreso",
        config={
            "LDAPURI": "ldap://172.16.200.202",
            "LDAPBASE": "dc=privacyidea," "dc=test",
            "BINDDN": "administrator@privacyidea.test",
            "BINDPW": "Test1234!",
            "HOSTNAMEATTRIBUTE": "dNSHostName",
        },
    )
    ...
    def test_01_get_config_description(self):
        ...
        self.assertEqual(desc.get("ldap").get("config").get("LDAPURI"), "string")
