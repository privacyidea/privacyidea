...
class APIPolicyTestCase(MyApiTestCase):
    ...
    def test_01_set_policy(self):
        ...
        with self.app.test_request_context(
            ...
            result = data.get("result")
            self.assertTrue(
                "setPolicy polpinode" in result.get("value"), result.get("value")
            )
    ...
    def test_03_set_adminuser_policy(self):
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(
                message,
                "Admin actions are defined, but you are not allowed to enroll this token type!",
            )
    ...
    def test_04_policy_defs(self):
        ...
        with self.app.test_request_context(
            "/policy/defs/pinodes", method="GET", headers={"Authorization": self.at}
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
            data = res.json
    ...
    def test_05_invalid_client(self):
        with self.app.test_request_context(
            ...
            self.assertEqual(
                result["error"],
                {"code": 302, "message": "ERR302: Invalid client definition!"},
            )
...
class APIPolicyConditionTestCase(MyApiTestCase):
    def test_01_check_httpheader_condition(self):
        ...
        with self.app.test_request_context(
            ...
            result = res.json
            self.assertIn(
                "Invalid comparison in the HTTP header conditions of policy",
                result["result"]["error"]["message"],
            )
        ...
        with self.app.test_request_context(
            ...
            json={
                ...
                "client": "10.1.2.3",
                "realm": "realm1",
                "conditions": [["blabla", "User-Agent", "equals", "SpecialApp", True]],
                "scope": SCOPE.AUTHZ,
            },
            headers={"PI-Authorization": self.at},
        ):
        ...
        with self.app.test_request_context(
            ...
            self.assertIn(
                "has condition with unknown section",
                result["result"]["error"]["message"],
                result,
            )
