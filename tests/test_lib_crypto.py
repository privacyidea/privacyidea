...
from privacyidea.lib.security.default import SecurityModule, DefaultSecurityModule
...
class SecurityModuleTestCase(MyTestCase):
    ...
    def test_04_random(self):
        config = current_app.config
        hsm = DefaultSecurityModule({"file": config.get("PI_ENCFILE"), "crypted": True})
        r = hsm.random(20)
    ...
    def test_07_encrypted_key_file(self):
        ...
        hsm = DefaultSecurityModule(
            {"file": config.get("PI_ENCFILE_ENC"), "crypted": True}
        )
        ...
        self.assertRaises(Exception, hsm.setup_module, {"password": "wrong PW"})
...
class CryptoTestCase(MyTestCase):
    ...
    def test_04_encrypt_decrypt_data(self):
        ...
        self.assertEqual(
            b"\x01\x02" * 30,
            decrypt(binascii.unhexlify(enc_data_hex), binascii.unhexlify(iv_hex)),
        )
...
class AESHardwareSecurityModuleTestCase(MyTestCase):
    ...
    def test_02_basic(self):
        with PKCS11Mock() as pkcs11:
            ...
            hsm.setup_module({"password": "test123!"})
            self.assertTrue(hsm.is_ready)
            self.assertEqual(pkcs11.mock.openSession.call_count, 1)
            self.assertIs(hsm.session, pkcs11.session_mock)
    ...
    def test_03_retry(self):
        with PKCS11Mock() as pkcs11:
            ...
            self.assertEqual(pkcs11.mock.openSession.mock_calls, [call(slot=1)])
            ...
            with pkcs11.simulate_failure(
                pkcs11.session_mock.generateRandom,
                5,
...
class AESHardwareSecurityModuleLibLevelTestCase(MyTestCase):
    ...
    def setUp(self):
        ...
        current_app.config["PI_HSM_MODULE"] = (
            "privacyidea.lib.security.aeshsm.AESHardwareSecurityModule"
        )
