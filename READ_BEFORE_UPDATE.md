...
still see all tokens. This restriction is currently only for listing token (`GET /token`). Other operations are not 
affected, so for example a helpdesk admin of realm A can still assign a token without a realm to a user in realm A, but
the serial of the token is required.
