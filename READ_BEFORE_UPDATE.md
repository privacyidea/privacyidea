...
* The `u2f` token is added to the list of not enrollable token types due to low technically relevance. Existing tokens 
  can still be used. The enrollment can be enabled by adding the type `u2f` to the configuration option list 
  `PI_ENABLE_TOKEN_TYPE_ENROLLMENT`.
