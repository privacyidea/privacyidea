...
def upgrade():
    try:
        ...
        op.create_index(
            op.f("ix_subscription_application"),
            "subscription",
            ["application"],
            unique=False,
        )
        op.alter_column("token", "active", existing_type=sa.BOOLEAN(), nullable=False)
        ...
        op.create_index(
            op.f("ix_token_tokentype"), "token", ["tokentype"], unique=False
        )
        op.create_index(op.f("ix_token_user_id"), "token", ["user_id"], unique=False)
        op.alter_column("tokenrealm", "id", existing_type=sa.INTEGER(), nullable=True)
    ...
    try:
        ...
        print(
            "## Unnecessary columns in table usercache obviously do not "
            "exist anymore."
        )
        print(exx)
...
def downgrade():
    ...
    op.create_index(
        "ix_usercache_expiration", "usercache", ["expiration"], unique=False
    )
    op.add_column("usercache", sa.Column("expiration", sa.DATETIME(), nullable=True))
    op.add_column(
        "usercache", sa.Column("realm", sa.VARCHAR(length=256), nullable=True)
    )

    # Remove Oracle Schema definition
    op.alter_column("tokenrealm", "id", existing_type=sa.INTEGER(), nullable=False)
    ...
    op.alter_column("token", "active", existing_type=sa.BOOLEAN(), nullable=True)
    ...
    op.create_index(
        "ix_clientapplication_id", "clientapplication", ["id"], unique=False
    )
