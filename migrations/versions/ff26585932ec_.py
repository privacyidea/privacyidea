...
def upgrade():
    ...
    try:
        for row in session.query(EventHandlerOption).filter(
            EventHandlerOption.Key == "reply_to"
        ):
            evh_id = row.eventhandler_id
            check = (
                session.query(EventHandlerOption)
                .filter(
                    and_(
                        EventHandlerOption.Key.like("reply_to %"),
                        EventHandlerOption.eventhandler_id == evh_id,
                    )
                )
                .first()
            )
