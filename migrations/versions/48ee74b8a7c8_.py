...
class Realm(Base):
    ...
    id = sa.Column(sa.Integer, Sequence("realm_seq"), primary_key=True, nullable=False)
...
class Token(Base):
    __tablename__ = "token"
    __table_args__ = {"mysql_row_format": "DYNAMIC"}
    id = sa.Column(sa.Integer, Sequence("token_seq"), primary_key=True, nullable=False)
    serial = sa.Column(
...
def upgrade():
    ...
    try:
        ...
        for token in session.query(Token).filter(
            Token.user_id != "", Token.user_id.isnot(None)
        ):
            token_realms = (
            ...
            if not token_realms:
                ...
                resolver = (
                    session.query(Resolver).filter_by(name=token.resolver).first()
                )
                if not resolver:
                    sys.stderr.write(
                        "{serial!s}, {userid!s}, {resolver!s}, "
