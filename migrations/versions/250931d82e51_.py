...
def upgrade():
    try:
        op.add_column(
            "pidea_audit",
            sa.Column("authentication", sa.Unicode(length=12), nullable=True),
        )
        op.add_column(
            "pidea_audit", sa.Column("user_agent", sa.Unicode(length=20), nullable=True)
        )
        op.add_column(
            "pidea_audit",
            sa.Column("user_agent_version", sa.Unicode(length=20), nullable=True),
        )
    except Exception as exx:
        print(
            "Can not add columns 'authentication', 'user_agent', 'user_agent_version' to "
