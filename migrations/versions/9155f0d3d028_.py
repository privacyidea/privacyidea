...
from privacyidea.models import Resolver, ResolverConfig
...
def upgrade():
    try:
        ...
        # set the TLS Version to v1.2 (=5) for all existing ldap resolvers
        for resolver_id in ldapresolvers_list:
            ...
            if (LDAPS or START_TLS):
                if TLS_VERSION_qry is None:
                    session.add(ResolverConfig(resolver_id=resolver_id, Key="TLS_VERSION", Value="5",
                                               Type=u'int'))
                elif TLS_VERSION == "":
                    base_query.filter(ResolverConfig.Key == "TLS_VERSION").update({"Value": "5"})
        session.commit()

    except Exception as exx:
        session.rollback()
        print("Failed to set TLS v1.2 for all existing ldap resolvers to prevent unexpected results.")
        print(exx)
