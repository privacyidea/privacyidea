...
class Resolver(Base):
    ...
    id = sa.Column(
        sa.Integer, Sequence("resolver_seq"), primary_key=True, nullable=False
    )
    name = sa.Column(sa.Unicode(255), default="", unique=True, nullable=False)
    rtype = sa.Column(sa.Unicode(255), default="", nullable=False)
    config_list = orm.relationship("ResolverConfig", lazy="select", backref="resolver")
...
class ResolverConfig(Base):
    ...
    __table_args__ = (
        sa.UniqueConstraint("resolver_id", "Key", name="rcix_2"),
...
def upgrade():
    try:
        ...
        for resolver_id in ldapresolvers_list:
            base_query = session.query(ResolverConfig).filter(
            ...
            TLS_VERSION_qry = base_query.filter(
                ResolverConfig.Key == "TLS_VERSION"
            ).first()
            if TLS_VERSION_qry:
                ...
                if TLS_VERSION_qry is None:
                    session.add(
                        ResolverConfig(
                            ...
                            Key="TLS_VERSION",
                            Value=DEFAULT_TLS_VERSION,
                            Type="int",
                        )
                    )
                elif TLS_VERSION == "":
                    base_query.filter(ResolverConfig.Key == "TLS_VERSION").update(
                        {"Value": DEFAULT_TLS_VERSION}
                    )
        session.commit()
        ...
        session.rollback()
        print(
            "Failed to set TLS v1.0 for all existing ldap resolvers to keep the legacy behavior."
        )
        print(exx)
