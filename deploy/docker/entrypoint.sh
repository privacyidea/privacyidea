...
# PI_DB_* vars match ConfigKey names used by DockerConfig; set in compose environment section.
if [ -z "${SQLALCHEMY_DATABASE_URI:-}" ] && [ -f /run/secrets/mariadb_password ]; then
    _db_pass=$(cat /run/secrets/mariadb_password)
    export SQLALCHEMY_DATABASE_URI="mysql+pymysql://${PI_DB_USER:-pi}:${_db_pass}@${PI_DB_HOST:-localhost}:${PI_DB_PORT:-3306}/${PI_DB_NAME:-pi}"
    unset _db_pass
fi
