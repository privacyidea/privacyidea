...
"""
Find and clean up orphaned rows in the ``customuserattribute`` table.
...
from privacyidea.lib.user import (
    delete_orphaned_custom_attributes,
    find_orphaned_custom_attributes,
)


@click.group('custom-attributes', invoke_without_command=True, cls=AppGroup)
...
@click.pass_context
def findcustomattributes(ctx, orphaned_on_error):
    """
    Find orphaned rows in the custom user-attribute table.
    ...
    ctx.obj['orphans'] = find_orphaned_custom_attributes(orphaned_on_error=orphaned_on_error)
    if ctx.invoked_subcommand is None:
        ctx.invoke(list_cmd)


@findcustomattributes.command('list')
@click.pass_context
def list_cmd(ctx):
    """List the orphaned (user_id, resolver, realm_id) tuples."""
    orphans = ctx.obj['orphans']
    if not orphans:
        click.echo("No orphaned custom user attributes found.")
        return
    click.echo(f"Found {len(orphans)} orphaned user(s) in customuserattribute:")
    for user_id, resolver, realm_id in orphans:
        click.echo(f"  user_id={user_id!r}  resolver={resolver!r}  realm_id={realm_id!r}")


@findcustomattributes.command('delete')
@click.option('--yes', is_flag=True, default=False,
              help="Skip the interactive confirmation.")
@click.pass_context
def delete_cmd(ctx, yes):
    """Delete every customuserattribute row that belongs to an orphaned user."""
    orphans = ctx.obj['orphans']
    if not orphans:
        click.echo("No orphaned custom user attributes found.")
        return
    if not yes:
        click.confirm(f"Delete all custom attributes for {len(orphans)} orphaned user(s)?",
                      abort=True)
    deleted = delete_orphaned_custom_attributes(orphans)
    click.echo(f"Deleted {deleted} customuserattribute row(s).")
