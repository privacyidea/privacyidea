...
@click.option(
    "--age", default=0, help="Delete audit entries older than these number of days."
)
@click.option(
...
def rotate_audit(highwatermark, lowwatermark, age, config, dryrun, chunksize):
    """
    ...
    if audit_module != default_module:
        click.secho(
            f"We only rotate SQL audit module. You are using {audit_module}", fg="red"
        )
    if config:
