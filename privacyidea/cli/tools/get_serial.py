...
@click.option(
    "-a", "--assigned", is_flag=True, help="If set, search only assigned tokens"
)
@click.option(
    "-u", "--unassigned", is_flag=True, help="If set, search only unassigned tokens"
)
@click.option(
    "-w",
    "--window",
...
@click.option(
    "-s", "--serial", type=str, default="", help="A part of the token serial number"
)
@click.option(
    "-t",
    "--type",
...
def byotp(otp, tokentype, serial, window, unassigned, assigned):
    ...
    tokenobj_list = get_tokens(
        tokentype=tokentype, serial_wildcard="*{0!s}*".format(serial), assigned=assigned
    )
