...
@click.option("-r", "--realm", default=None, help="The realm which should be checked")
@click.option(
    "-a",
    "--attribute_name",
...
def expire(realm, attribute_name, delete_serial, unassign_serial, noaction):
    ...
    try:
        ...
        click.secho(
            f"Does the attribute '{attribute_name}' exist in the "
            f"attribute mapping of the resolver?",
            fg="yellow",
        )
        users = []
    ...
    for user in users:
        ...
        if account_expires:
            ...
            if account_expires <= utc_now:
                ...
                tokens = get_tokens(
                    user=User(login=username, resolver=user["resolver"])
                )
                if not tokens:
                ...
                for token in tokens:
                    if unassign_serial:
                        ...
                        if m:
                            if noaction:
                                click.echo(
                                    f"=== I WOULD unassign token {token.token.serial}"
                                )
                            else:
                                click.echo(
                                    f"=== Unassigning token {token.token.serial}"
                                )
                    ...
                    if delete_serial:
                        ...
                        if m:
                            if noaction:
                                click.echo(
                                    f"=== I WOULD delete token {token.token.serial}"
                                )
                            else:
