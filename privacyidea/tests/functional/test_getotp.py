...
#
#  2014-10-02 Add more tests to cover controller /gettoken
...
class TestGetOtpController(TestController):
    '''
    This test at the moment only tests the implementation for the Tagespasswort
    Token
    ...
    def test_01_getotp_dpw(self):
        '''
        get otp value of the DPW token
        '''
        parameters = {'serial': 'dpw1',
                      'curTime': self.curTime,
                      'selftest_admin': 'admin'
                      }
        response = self.app.get(url(controller='gettoken',
                                    action='getotp'),
                                params=parameters)
    ...
    def test_02_getotp_policyfail(self):
        '''
        get otp fails due to policy
        '''
        parameters = {'serial': 'dpw1',
                      'curTime': self.curTime,
                      'selftest_admin': 'admin2'
                      }
        response = self.app.get(url(controller='gettoken',
                                    action='getotp'),
                                params=parameters)
        ...
        assert 'The policy forbids receiving OTP values' in response
    ...
    def test_03_getmultiotp(self):
        ...
        response = self.app.get(url(controller='gettoken',
                                ...
                                params=parameters)
        print response
        assert '<td class="getotp key">12-05-16</td>' in response
        assert '<td class="getotp value">427701</td>' in response
        
    def test_04_getmultiotp(self):
        '''
        get multi otp fails due to policy
        '''
        parameters = {'serial': 'dpw1',
                      'curTime': self.curTime,
                      'count': "10",
                      'selftest_admin': 'looseradmin'
                      }
        response = self.app.get(url(controller='gettoken',
                                    action='getmultiotp'),
                                params=parameters)
        ...
        assert 'No count specified' in response

    def test_05_getotp_hotp(self):
        '''
        get otp value of the HOTP token
        '''
    ...
    def test_05a_gettoken_by_user(self):
        ...
        # assign the second token to the user
        response = self.app.get(url(controller='admin',
                                    action='assign'),
                                params={'serial': 'dpw1',
                                        'user': 'user1'})
        ...
        # clean up: unassign token
        for serial in ["dpw1", "hotp1"]:
            response = self.app.get(url(controller='admin',
                                        action='unassign'),
    ...
    def test_00_missing_parameter(self):
        '''
        get otp missing parameter
        '''
        response = self.app.get(url(controller='gettoken',
                                    action='getotp'),
                                params={})
        print response
        assert('"description": "you need to provide a user or a serial"'
               in response)
        
    def test_00_user_without_token(self):
        '''
        get otp from user without token
        '''
        response = self.app.get(url(controller='gettoken',
                                    action='getotp'),
                                params={'user': 'horst'})
        print response
        assert('"description": "No Token found for this user"'
               in response)
        
    def test_00_wrong_serial(self):
        '''
        get otp from wrong serial
        '''
        response = self.app.get(url(controller='gettoken',
                                    action='getotp'),
                                params={'serial': '123XXX'})
        print response
        assert('ERR1119: getTokenRealms failed. No token with serial '
               '123XXX found' in response)
        
    def test_00_token_without_getotp(self):
        '''
        get otp from token, not supporting getOtp
        '''
        response = self.app.get(url(controller='admin', action='init'),
                                params={'serial': 'spass1',
                                        'type': 'spass'})
        print response
        assert '"status": true' in response
        assert '"value": true' in response
        ...
        response = self.app.get(url(controller='gettoken',
                                ...
                                params={'serial': 'spass1'})
        print response
        assert('ERR410: There is no policy gettoken/max_count definable '
               'for the tokentype' in response)
    ...
    def test_07_getotp_totp(self):
        '''
        get otp value of the TOTP token
    ...
    def test_09_usergetmultiotp_no_policy(self):
        '''
        get otp value for a users own token with missing policy
        '''
        parameters = {'serial': 'totp1',
                      'timestamp': self.TOTPTimestamp,
                      'count': "20",
    ...
    def test_10_usergetmultiotp(self):
        '''
        get otp value for a users own token
        '''
    ...
    def test_11_usergetmultiotp_fail(self):
        '''
        get otp value for a  token that does not belong to the user
        '''
        parameters = {'serial': 'hotp1',
                      'timestamp': self.TOTPTimestamp,
                      'count': "20",
        ...
        response = self.app.get(url(controller='selfservice',
                                ...
                                params=parameters)
        print response
        assert('"message": "The serial hotp1 does not belong to user '
               'localuser@mydefrealm"' in response)
