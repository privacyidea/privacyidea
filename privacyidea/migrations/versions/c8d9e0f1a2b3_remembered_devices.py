...
def upgrade():
    try:
        op.create_table(
            ...
            # so no database sequence is required. It is the secret half of the
            # cookie and is never exposed; device_id is the non-secret handle used
            # to list and revoke a device.
            sa.Column('series_id', sa.Unicode(length=64), nullable=False),
            sa.Column('device_id', sa.Unicode(length=64), nullable=False),
