...
class NodeName(db.Model, TimestampMethodsMixin):
    ...
    lastseen = db.Column(
        db.DateTime(False), index=True, default=datetime.now(tz=tzutc())
    )
...
class ResolverRealm(TimestampMethodsMixin, db.Model):
    ...
    node = db.relationship(NodeName, lazy="joined")
    __table_args__ = (
        db.UniqueConstraint("resolver_id", "realm_id", "node_uuid", name="rrix_2"),
        {"mysql_row_format": "DYNAMIC"},
    )
    ...
    def __init__(
        ...
        if node_uuid:
            ...
            self.node_uuid = (
                NodeName.query.filter_by(name=node_name)
...
class PrivacyIDEAServer(MethodsMixin, db.Model):
    ...
    __table_args__ = {"mysql_row_format": "DYNAMIC"}
    id = db.Column(db.Integer, Sequence("privacyideaserver_seq"), primary_key=True)
    ...
    def save(self):
        pi = PrivacyIDEAServer.query.filter(
        ...
        if pi is None:
            ...
            PrivacyIDEAServer.query.filter(
                PrivacyIDEAServer.identifier == self.identifier
            ).update(values)
            ret = pi.id
        db.session.commit()
...
class RADIUSServer(MethodsMixin, db.Model):
    ...
    * retries (default 3)
    ...
    dictionary = db.Column(db.Unicode(255), default="/etc/privacyidea/dictionary")
    description = db.Column(db.Unicode(2000), default="")
...
class Audit(MethodsMixin, db.Model):
    ...
    user_agent_version = db.Column(
        db.Unicode(audit_column_length.get("user_agent_version"))
    )
