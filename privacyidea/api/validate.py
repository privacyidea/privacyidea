...
def offlinerefill():
    ...
    try:
        ...
        if len(tokens) != 1:
            raise ParameterError(lazy_gettext("The token does not exist"))
        ...
        if not token.check_all(message_list):
            ...
            raise ParameterError(lazy_gettext("The token is not valid."))
        ...
        if token_attachments:
            ...
            if token.type.lower() == "hotp":
                ...
                if not computer_name:
                    ...
                    raise ParameterError(lazy_gettext("Machine can not be identified by user agent!"))
            ...
            if refilltoken_stored and refilltoken_stored == refilltoken_request:
                ...
                return response
        raise ParameterError(lazy_gettext("Token is not an offline token or refill token is incorrect"))
...
def _handle_fido2_auth(context: dict, credential_id: str):
    ...
    if (PolicyAction.DISABLED_TOKEN_TYPES in request.all_data and
        ...
        raise PolicyError(lazy_gettext("The authentication method is not available."))
    ...
    if attestation_object:
        ...
        if not check_last_auth_policy(g, token):
            ...
            context["details"]["message"] = lazy_gettext(
    ...
    if context["result"]:
        context["details"].update({
            ...
            "message": lazy_gettext("Found matching challenge"),
            "serial": token.get_serial()
        })
        context["serial_list"].append(token.get_serial())
    else:
        context["details"]["message"] = lazy_gettext("Authentication failed.")
...
def _handle_serial_auth(context: dict, serial: str):
    ...
    if user:
        try:
            ...
            if not tokens:
                raise ParameterError(lazy_gettext("Given serial does not belong to given user!"))
...
def initialize():
    ...
    if token_type.lower() == "passkey":
        ...
        raise ParameterError(lazy_gettext("Unsupported token type '{token_type}' for authentication initialization!").format(token_type=token_type))
