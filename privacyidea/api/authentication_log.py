...
_FILTER_PARAMS = ["resolver", "uid", "realm", "username", "user_role", "event_type", "source_ip", "serial",
                  "transaction_id", "previous_transaction_id", "attempt_id", "client_label"]
...
def get_authentication_log():
    ...
    ``transaction_id``, ``previous_transaction_id``, ``attempt_id`` and ``client_label`` may be passed as a query
    ...
    :query sort_column: column to sort by (id, timestamp, event_type, resolver, uid, realm, username, source_ip,
        client_label, serial, transaction_id, previous_transaction_id, attempt_id).
