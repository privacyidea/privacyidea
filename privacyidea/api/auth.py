...
from privacyidea.lib.auth import check_webui_user, ROLE, verify_db_admin, db_admin_exist
...
def get_auth_token():
    ...
    **Response for failed authentication**:
    ...
    if username is None:
        raise AuthError(
            _("Authentication failure. Missing Username"),
    ...
    if not admin_auth and not user_auth:
        raise AuthError(
            _("Authentication failure. Wrong credentials"),
            id=ERROR.AUTHENTICATE_WRONG_CREDENTIALS,
            details=details or {},
        )
    else:
