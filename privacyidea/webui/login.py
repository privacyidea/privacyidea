...
def single_page_application():
    ...
    if len(customization_menu_file) and customization_menu_file.keys()[0] \
            and sub_state not in [1, 2]:
        customization_menu_file = customization_menu_file.keys()[0]
    else:
    ...
    customization_baseline_file = policy_object.get_action_values(
        ...
        scope=SCOPE.WEBUI,
        client=client_ip, unique=True)
    if len(customization_baseline_file) and customization_baseline_file.keys()[0] \
            and sub_state not in [1, 2]:
        customization_baseline_file = customization_baseline_file.keys()[0]
    else:
