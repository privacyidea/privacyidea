...
def get_firebase_access_token(config_file_name):
    ...
    if (
        ...
        log.debug(
            "Setting the expiration for {!r} of the new access_token "
...
class FirebaseProvider(ISMSProvider):
    ...
    def submit_message(self, firebase_token, data):
        ...
        if resp.status_code == 200:
            ...
            log.warning(
                "Failed to send message to firebase service: {0!s}".format(resp.text)
            )
        ...
        params = {
            ...
            "parameters": {
                FIREBASE_CONFIG.JSON_CONFIG: {
                    "required": True,
                    "description": _(
                        "The filename of the JSON config file, that allows privacyIDEA to talk"
