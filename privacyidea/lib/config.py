        ...
        for name in dir(module):
            ...
            if (
                ...
                try:
                    ...
                    log.error(
                        "error constructing machine resolver " "class_list: %r" % e
                    )
        ...
        for name in dir(module):
            ...
            if (
                ...
                try:
                    ...
                    log.error("error constructing CA connector " "class_list: %r" % e)
...
def get_caconnector_module_list():
    ...
    for mod_name in module_list:
        ...
        try:
            ...
            module = None
            log.warning(
                "unable to load ca connector module : {0!r} ({1!r})".format(
                    mod_name, exx
                )
            )
...
def get_machine_resolver_module_list():
    ...
    for class_name in class_list:
        try:
            ...
            module = None
            log.warning(
                "unable to load machine resolver module : {0!r} ({1!r})".format(
                    module_name, exx
                )
            )
...
def get_privacyidea_node(default="localnode"):
    ...
    node_name = get_app_config_value(
        "PI_NODE", get_app_config_value("PI_AUDIT_SERVERNAME", default)
    )
    return node_name
