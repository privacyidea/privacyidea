...
class HotpTokenClass(TokenClass):
    ...
    def get_init_detail(self, params=None, user=None):
        ...
        if is_true(params.get("2stepinit")):
            response_detail["2step"] = self._construct_2step_parameters(params)
    ...
    def generate_symmetric_key(self, server_component, client_component,
        ...
        :type server_component: hex string
        :param options: a dictionary containing extra options:
          * ``2step_difficulty``: number of PBKDF2 rounds
          * ``2step_clientsize``: used to validate the length of ``client_component``
        :return: the new generated key as hex string
        """
        # As /token/init has already been called before, self.hashlib
        # is already set.
        keysize = keylen[self.hashlib]
        rounds = int(options['2step_difficulty'])
        ...
        # as the salt.
        secret = pbkdf2(decoded_server_component,
