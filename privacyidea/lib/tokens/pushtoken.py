...
def _build_smartphone_data(token, challenge, registration_url, private_key_pem, options,
    ...
    :param push_code_to_phone_code: The OTP value that the user has to enter in the login prompt.
    :type push_code_to_phone_code: str
    ...
    if push_code_to_phone_code and presence_options is None:
        default_message = DEFAULT_MOBILE_TEXT_CODE_TO_PHONE
    ...
    elif push_code_to_phone_code:
        smartphone_data["display_code"] = push_code_to_phone_code
        smartphone_data["version"] = "3"
        sign_string += f"|{push_code_to_phone_code}"
        ...
        try:
            ...
            for challenge in db_challenges:
                ...
                if not answered and challenge.is_valid():
                    ...
                    # todo add mode to smartphone data, check format etc
                    ...
                    smartphone_data = _build_smartphone_data(token, challenge.challenge, registration_url, private_key,
                                                             options, presence_options, code_to_phone)
