...
from privacyidea.models import (AuthenticationLog, BlockList, ConditionalAccessOutcome, ConditionalAccessPolicy,
                                ConditionalAccessPolicyCounterType, ConditionalAccessPolicyStage, ConditionalAccessStageAction, UserLockState)
...
class AccessDecisionResult:
    ...
    (:func:`evaluate_access_decision`), the way :class:`ConditionalAccessEvaluation` serves one policy and all of them. Hence the
...
def _policy_count(policy: ConditionalAccessPolicy, user: "User", window_end: datetime,
    ...
    policy's :attr:`~privacyidea.models.conditional_access_policy.ConditionalAccessPolicy.count_mode`, scoped to the rows the policy's
...
def evaluate_conditional_access_policies(context: CAContext, event_type: AuthEventType | None,
    ...
    :param event_type: the classified outcome of the request
        (:class:`AuthEventType`)
    :param now: the reference time; defaults to :func:`utc_now`
    :return: a :class:`ConditionalAccessEvaluation` holding the de-duplicated, order-preserving user-facing notices produced by
...
def _evaluate_policy(policy: ConditionalAccessPolicy, context: CAContext, event_type: str,
    ...
    :return: a :class:`ConditionalAccessEvaluation` with the user-facing notices produced by the executed actions and the
...
def _base_action_tags(policy: ConditionalAccessPolicy, stage: ConditionalAccessPolicyStage, context: CAContext, event_type: str,
...
def _execute_stage_actions(policy: ConditionalAccessPolicy, stage: ConditionalAccessPolicyStage,
    ...
    :return: a :class:`ConditionalAccessEvaluation` with the user-facing notices produced by executed ``EMAIL_*`` actions and one
...
def _delete_ip_block(state: BlockList) -> None:
    ...
    :func:`_delete_user_lock_state`: used by :func:`get_ip_block` to drop a
