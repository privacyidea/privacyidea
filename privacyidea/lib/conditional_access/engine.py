...
def evaluate_access_decision(context: CAContext, now: datetime | None = None) -> "AccessDecisionResult":
    ...
    :attr:`LockoutAction.LOCK_USER_TEMPORARY`). Because ``DENY`` defaults to
    re-triggering (``count >= threshold``), a stage with ``failure_threshold`` 0
    always matches, which is the lockdown idiom - scope it with conditions.
...
def _evaluate_policy(policy: LockoutPolicy, context: CAContext, event_type: str,
    ...
    if policy.dry_run:
        ...
        # surfaces that too.
        # DENY is left out: it decides the request pre-auth and is recorded there (_policy_access_decision), so
        # recording it again here would double-count the same decision.
