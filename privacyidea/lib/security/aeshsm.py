...
class AESHardwareSecurityModule(SecurityModule):  # pragma: no cover
    ...
    def __init__(self, config=None):
        ...
        for k in self.mapping:
            label = config.get(f"key_label_{k!s}")
            label = (f'{label_prefix!s}_{k!s}') if label is None else label
            self.key_labels[k] = label
    ...
    def _login(self):
        ...
        log.debug(f"Starting new session now. The old session started "
                  f"{datetime.datetime.now() - self.session_start_time!s} seconds ago.")
        log.debug(f"Starting new session now. The old session was used "
                  f"{datetime.datetime.now() - self.session_lastused_time!s} seconds ago.")
