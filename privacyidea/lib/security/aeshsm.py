...
import logging
import datetime
from privacyidea.lib.security.default import SecurityModule, int_list_to_bytestring
...
class AESHardwareSecurityModule(SecurityModule):  # pragma: no cover
    ...
    def decrypt(self, enc_data, iv, key_id=SecurityModule.TOKEN_KEY):
            ...
            try:
                ...
                if exx.value == PyKCS11.CKR_SESSION_HANDLE_INVALID:
                    ...
                    if retries > self.max_retries:
                        ...
                        log.warning(
                            "Decryption finally failed: {0!s}. Time taken: {1!s}.".format(
                                exx, td
                            )
                        )
        ...
        if retries > 0:
            td = datetime.datetime.now() - start
            log.warning(
                "Decryption after {0!s} retries successful. Time taken: {1!s}.".format(
                    retries, td
                )
            )
        return int_list_to_bytestring(r)
...
if __name__ == "__main__":  # pragma: no cover
    ...
    p = AESHardwareSecurityModule(
        {"module": module, "slot": 2, "key_label": "privacyidea"}
    )
