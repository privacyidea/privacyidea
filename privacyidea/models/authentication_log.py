...
class AuthenticationLog(MethodsMixin, db.Model):
    ...
    attempt's rows by ``(timestamp, id)`` reconstructs the full chain, and each row's own ``transaction_id`` still
    links back to the challenge table.

    ``timestamp`` leads that order rather than ``id``, and carries microsecond precision for it, because ``id`` is a
    plain autoincrement: it is the insertion order of a *single writer* only. A multi-master cluster (Galera with
    ``wsrep_auto_increment_control``) gives each node its own offset and applies remote writes asynchronously, so a
    row committed later on one node can carry a lower ``id`` than one committed earlier on another - which would let
    a counted failure vanish or a counter reset be missed. ``timestamp`` is generated by the application, so it orders
    across writer nodes; ``id`` stays the tiebreaker and is exact for the rows of one flush. The residual limit is the
    clock offset between privacyIDEA instances: events that fall within it are ordered by ``id``, i.e. arbitrarily -
    which is no worse than they are distinguishable.
    ...
    **NULL in any of the four provenance columns means the row predates them**, and nothing may be inferred
    ...
    __table_args__ = (
        ...
        # with no event_type predicate, so each needs timestamp right after the subject column(s).
        Index("ix_authlog_user_time", "resolver", "uid", "realm", "timestamp"),
        Index("ix_authlog_ip_time", "source_ip", "timestamp"),
        # The subject index for a local database admin, who has no resolver, uid or realm to be found by: their
        # rows carry only the login name, and the role is what separates them from a same-named user's (see
        # LockSubject in lib.conditional_access.engine). Both conditional-access counts key on that pair, so
        # without this every failed local-admin login would scan the table. event_type before timestamp mirrors
        # ix_authlog_user_event_time; the PER_ATTEMPT count, which has no event_type predicate, range-scans the
        # leading two columns.
    ...
    # Microseconds on every backend, because the reduction in
    # :func:`~privacyidea.lib.conditional_access.engine._count_matching_attempts` orders an attempt's rows by time
    # rather than by id.
    timestamp: Mapped[datetime] = mapped_column(
        DateTime().with_variant(mysql.DATETIME(fsp=6), "mysql", "mariadb").with_variant(oracle.TIMESTAMP(), "oracle"),
