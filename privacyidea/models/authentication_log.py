...
class AuthenticationLog(MethodsMixin, db.Model):
    ...
    **NULL in any of the four provenance columns means the row predates them**, and nothing may be inferred
    ...
    __table_args__ = (
        ...
        # (:func:`~privacyidea.lib.conditional_access.authentication_log_statistics.get_authentication_log_statistics`)
        # and the retention delete (``cleanup_authentication_log``) both range over ``timestamp`` alone, and the indexes
