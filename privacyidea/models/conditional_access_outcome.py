...
conditional_access_outcome_column_length = {
    # Mirrors conditional_access_stage_actions.action_type, whose vocabulary (ConditionalAccessAction) this column stores.
    "action_type": 100,
...
class ConditionalAccessOutcome(db.Model):
    ...
    auth_log_id: Mapped[int] = mapped_column(BigIntegerType,
                                             ...
                                             nullable=False)
    # A ConditionalAccessAction value: the action that ran (LOCK_USER, BLOCK_IP, EMAIL_*, ...) or the pre-auth decision (DENY).
