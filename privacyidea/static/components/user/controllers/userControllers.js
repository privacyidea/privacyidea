 ...
 *
 */
angular.module("privacyideaApp").controller("userAddController", [
  "$scope",
  "userUrl",
  "$state",
  "$location",
  ...
  function (
    ...
    $scope.createUser = function () {
      ...
      UserFactory.createUser($scope.resolvername, $scope.User, function (data) {
        //debug: console.log(data.result);
        inform.add(gettextCatalog.getString("User created."), { type: "info" });
...
angular.module("privacyideaApp").controller("userPasswordController", [
  "$scope",
  "userUrl",
  "UserFactory",
  "inform",
...
angular.module("privacyideaApp").controller("userDetailsController", [
  "$scope",
  "userUrl",
  "realmUrl",
  "tokenUrl",
  ...
  function (
    ...
    $scope._getUserToken = function () {
      TokenFactory.getTokenForUser(
        ...
        function (data) {
          ...
          $scope.enableAddTokenToContainer =
            $scope.tokenWithoutContainer.length > 0;
        },
      );
    };
    ...
    $scope.addCustomAttribute = function () {
      ...
      UserFactory.setCustomAttribute(
        $scope.username,
        $scope.realmname,
        key,
        value,
    ...
    $scope.getCustomAttributes = function () {
      UserFactory.getCustomAttributes(
        $scope.username,
        $scope.realmname,
        function (data) {
    ...
    $scope.canReadInternalAttributes = AuthFactory.checkRight(
      "get_user_internal_attributes",
    );
    $scope.internalAttributesLoaded = false;
    $scope.internal_attributes = {};
    $scope.accordionState = { internalAttributesOpen: false };
    $scope.formatInternalAttribute = function (value) {
      return typeof value === "string" ? value : JSON.stringify(value);
    };
    $scope.$watch("accordionState.internalAttributesOpen", function (isOpen) {
      if (isOpen && !$scope.internalAttributesLoaded) {
        UserFactory.getInternalAttributes(
          $scope.username,
          $scope.realmname,
          function (data) {
    ...
    $scope.deleteCustomAttribute = function (key) {
      UserFactory.deleteCustomAttribute(
        $scope.username,
        $scope.realmname,
        key,
        function (data) {
    ...
    $scope.onCustomAttributeKeyChange = function () {
      ...
      $scope.allowed_values =
        $scope.allowedCustomAttributes["set"][$scope.selectedAttrKey];
    ...
    $scope.updateUser = function () {
      UserFactory.updateUser($scope.resolvername, $scope.User, function (data) {
        if (data.result.value) {
          inform.add(
            gettextCatalog.getString("User updated " + "successfully."),
            { type: "info" },
          );
          ...
          inform.add(gettextCatalog.getString("Failed to update user."), {
            type: "danger",
          });
        }
      });
    ...
    $scope.deleteUser = function () {
      UserFactory.deleteUser(
        ...
        function (data) {
          if (data.result.value) {
            inform.add(
              gettextCatalog.getString("User deleted " + "successfully."),
              { type: "info" },
            );
            $scope._getUsers();
            $location.path("/user/list");
          } else {
            inform.add(gettextCatalog.getString("Failed to delete user."), {
              type: "danger",
            });
          }
        },
    ...
    $scope.enrollToken = function () {
      ...
      $rootScope.returnTo =
        "user.details({realmname:$scope.realmname, username:$scope.username})";
    };
...
angular.module("privacyideaApp").controller("userListController", [
  "$scope",
  function ($scope) {
    // Change the pagination
...
angular.module("privacyideaApp").controller("userController", [
  "$scope",
  "$location",
  "userUrl",
  "realmUrl",
  ...
  function (
    ...
    $scope._getUsers = function (live_search) {
      if (
        ...
        ($rootScope.search_on_enter && !live_search)
      ) {
      ...
      for (const rname in resolvers) {
        ...
        switch (resolver.type) {
          case "ldapresolver":
            ...
            if (
              resolver.data.recursive_group_search &&
              resolver.data.group_attribute_mapping_key
            ) {
          ...
          case "httpresolver":
            ...
            delete userinfo["userid"];
            if (
              resolver.data.config_get_user_groups &&
              resolver.data.config_get_user_groups.active
            ) {
              const groups_key =
                resolver.data.config_get_user_groups.pi_user_groups_key ||
                "groups";
              userinfo[groups_key] = [];
            }
            break;
