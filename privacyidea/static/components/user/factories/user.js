...
myApp.factory("UserFactory", [
  ...
  "inform",
  "$q",
  function (AuthFactory, $http, $state, $rootScope, userUrl, inform, $q) {
    ...
    return {
      ...
      deleteUser: function (resolver, username, callback) {
        $http
          .delete(
            userUrl + "/" + resolver + "/" + encodeURIComponent(username),
            { headers: { "PI-Authorization": AuthFactory.getAuthToken() } },
          )
          .then(
            function (response) {
      ...
      getEditableAttributes: function (params, callback) {
        $http
          .get(userUrl + "/editable_attributes/", {
            headers: { "PI-Authorization": AuthFactory.getAuthToken() },
            params: params,
          })
          .then(
      ...
      getInternalAttributes: function (username, realmname, callback) {
        const params = { user: username, realm: realmname };
        $http
          .get(userUrl + "/internal_attribute", {
            headers: { "PI-Authorization": AuthFactory.getAuthToken() },
            params: params,
          })
          .then(
