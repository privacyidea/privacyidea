...
export class TokenService implements TokenServiceInterface {
  ...
  tokenTypeOptions = computed<TokenType[]>(() => {
    ...
    return Object.entries(obj).map(([key, info]) => ({
      ...
      info: String(info),
      text: TokenService.tokenTypes.find((t) => t.key === key)?.text || ""
    }));
  });
  ...
  enrollmentAllowed(): boolean {
    const enrollPolicies = TokenService.tokenTypes.map((type) => "enroll" + type.key.toUpperCase());
    return this.authService.oneActionAllowed(enrollPolicies);
  }
}
