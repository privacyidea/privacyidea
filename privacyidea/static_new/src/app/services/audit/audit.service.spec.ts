...
class MockContentService implements Partial<ContentService> {
  routeUrl = signal('/home');
  selectedContent = signal<'token_overview' | 'audit'>('token_overview');
}
...
describe('AuditService (signals & helpers)', () => {
  ...
  it('changing filterValue recomputes sort & leaves pageSize at 10', () => {
    expect(auditService.pageSize()).toBe(10);
    expect(auditService.sort().active).toBe('serial');

    auditService.filterValue.set({ action: 'LOGOUT' });
    expect(auditService.pageSize()).toBe(10);
    expect(auditService.sort().active).toBe('serial');
  ...
  it('auditResource builds a request when route or tab is audit', () => {
    ...
    content.routeUrl.set('/home');
    content.selectedContent.set('audit');
    auditService.auditResource.reload();
    expect(local.getHeaders).toHaveBeenCalledTimes(2);
  });
