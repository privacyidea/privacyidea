    ...
    return this.http
      ...
      .pipe(
        catchError((error) => {
          ...
          this.notificationService.error($localize`:@@user.failedSetUser:Failed to set user attribute. ` + message);
          return of(undefined);
        })
      );
  }
    ...
    return this.http.delete<PiResponse<number>>(url, { headers: this.authService.getHeaders() }).pipe(
      catchError((error) => {
        ...
        this.notificationService.error(
          $localize`:@@user.failedDeleteUserAttribute:Failed to delete user attribute. ` + message
        );
        return of(undefined);
      })
    );
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError((error) => {
          ...
          const message = error.error?.result?.error?.message || "";
          this.notificationService.error(
            $localize`:@@user.failedCreateUser:Failed to create user ${userData.username}. ` + message
          );
          return of(false);
        })
      );
  ...
  editUser(resolver: string, userData: EditUserData) {
    ...
    return this.http.put<PiResponse<number>>(this.baseUrl, payload, { headers: this.authService.getHeaders() }).pipe(
      ...
      catchError((error) => {
        ...
        const message = error.error?.result?.error?.message || "";
        this.notificationService.error(
          $localize`:@@user.failedUpdateUser:Failed to update user ${userData.username}. ` + message
        );
        return of(false);
      })
    );
  ...
  deleteUser(resolver: string, username: string): Observable<boolean> {
    ...
    return this.http.delete<PiResponse<number>>(url, { headers: this.authService.getHeaders() }).pipe(
      ...
      catchError((error) => {
        ...
        const message = error.error?.result?.error?.message || "";
        this.notificationService.error(
          $localize`:@@user.failedDeleteUser:Failed to delete user ${username}. ` + message
        );
        return of(false);
      })
    );
