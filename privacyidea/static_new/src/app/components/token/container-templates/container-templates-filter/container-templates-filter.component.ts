...
export class ContainerTemplatesFilterComponent {
   ...
   */
  @Input() set initialFilter(value: FilterValueGeneric<ContainerTemplate>) {
    if (value) {
  ...
  readonly filter = signal<FilterValueGeneric<ContainerTemplate>>(new FilterValueGeneric({ availableFilters: [] }));
  readonly isEmpty = signal(true);

  // The raw string bound to the input [value]
  public lastFilter: FilterValueGeneric<ContainerTemplate> | null = null;

  /**
   * Manually updates the filter state.
   * Uses a guard to prevent redundant signal updates and DOM jitter.
   */
  public updateFilterManually(newFilter: FilterValueGeneric<ContainerTemplate>): void {
