...
A manual restriction is stored in the same place a policy writes to and is
enforced by the same pre-check, so it behaves exactly like a policy lock at
authentication time and is cleared by the same *Unlock* / *Unblock* action.
What is recorded alongside it is its **cause**: the *Locked Users* and
*Blocklist* pages show a *Cause* column reading *Manual* or *Policy*, and the
locked-users list can be filtered on it. The cause always describes the
restriction currently in force - if a policy later strengthens a manual lock,
the row becomes a policy lock.
...
itself. A never-block IP address (loopback, or one covered by
